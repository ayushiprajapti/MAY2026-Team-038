...
export default function EventRegistration() {
  ...
  if (!getToken()) {
    return <main className="min-h-[55vh] bg-[#f7eddc] px-4 py-16 text-center"><h1 className="font-serif text-3xl text-[#422b1b]">Log in to register</h1><p className="mt-3 font-body text-[#755d48]">Please sign in to register for <strong>{event.title}</strong>.</p><Link to="/login" className="mt-6 inline-block rounded-lg bg-[#b87519] px-5 py-3 font-sans text-sm font-bold text-white">Log in</Link></main>;
  }

  if (submitted) {
    return <main className="min-h-[55vh] bg-[#f7eddc] px-4 py-16 sm:px-6"><section className="mx-auto max-w-xl rounded-2xl border border-[#b9c29f] bg-[#fffaf0] p-8 text-center shadow-[0_12px_32px_rgba(96,69,30,0.1)] sm:p-12"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#e7eedc] text-3xl text-[#526348]">✓</div><p className="mt-6 font-sans text-xs font-bold uppercase tracking-[0.2em] text-[#6c804f]">{registrationStatus === "waitlisted" ? "Added to waitlist" : "Registration complete"}</p><h1 className="mt-2 font-serif text-3xl text-[#422b1b]">{registrationStatus === "waitlisted" ? "You're on the waitlist." : "You're on the list."}</h1><p className="mt-4 font-body leading-7 text-[#755d48]">{registrationStatus === "waitlisted" ? <>This event is full. We'll email you if a seat opens up for <strong>{event.title}</strong>.</> : <>Thank you for registering for <strong>{event.title}</strong>. We'll send your event details to the email address you provided.</>}</p><button onClick={() => navigate("/events")} className="mt-7 rounded-lg bg-[#b87519] px-5 py-3 font-sans text-sm font-bold text-white hover:bg-[#925a0e]">Back to events</button></section></main>;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const registration = await registerForEvent(event.id, {
        ...
        receive_event_updates: form.get("receive_event_updates") === "on",
      });
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,.65fr)] lg:items-start">
          <section className="order-2 rounded-2xl border border-[#dfcda8] bg-[#fffaf0] p-5 shadow-[0_10px_28px_rgba(96,69,30,0.08)] sm:p-8 lg:order-1">
            ...
            {error && <p className="mt-4 rounded-lg border border-[#e3b48c] bg-[#fbe9db] px-4 py-3 font-sans text-sm text-[#8a4527]">{error}</p>}
            <form onSubmit={handleSubmit} className="mt-7 space-y-5">
              ...
              <label className="font-sans text-sm font-bold text-[#533824]">Number of attendees *<input required type="number" name="attendees" min="1" max="20" defaultValue="1" inputMode="numeric" className="mt-2 block w-full rounded-lg border border-[#d9c29d] bg-[#fffdf8] px-3 py-2.5 font-normal outline-none focus:ring-2 focus:ring-[#d39834]" /></label>
              ...
              <label className="flex items-center gap-3 font-sans text-sm leading-5 text-[#684f3a]">
                <input type="checkbox" name="receive_event_updates" className="h-5 w-5 shrink-0 rounded border-[#b99667] accent-[#ad741d]" />
                <span>I agree to receive event-related updates from INTACH Pune.</span>
              </label>
              <div className="border-t border-[#eadcc2] pt-5"><button type="submit" disabled={submitting} className="w-full rounded-lg bg-[#b87519] px-5 py-3 font-sans text-sm font-bold text-white shadow-sm transition hover:bg-[#925a0e] focus:outline-none focus:ring-2 focus:ring-[#d39834] focus:ring-offset-2 disabled:opacity-60 sm:w-auto">{submitting ? "Submitting…" : "Confirm registration"}</button></div>
            </form>
          </section>
