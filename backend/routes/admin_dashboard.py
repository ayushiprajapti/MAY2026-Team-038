...
def get_shop_stats(conn: connection = Depends(get_db)):
    return dashboard_service.get_shop_stats(conn)


@router.get("/events", response_model=DashboardEventsResponse)
def get_dashboard_events(conn: connection = Depends(get_db)):
...
def get_recent_uploads(conn: connection = Depends(get_db)):
